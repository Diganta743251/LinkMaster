...
object AppPreferences : PreferenceDefinition(
    ...
    val linkHistoryDurationDays = int("link_history_duration_days", -1) // -1 = disabled
    val linkHistoryMaxCount = int("link_history_max_count", -1) // -1 = no limit
...
//    val lastVersionsV1 = jsonMapped<LastVersion?>("last_versions", null)
    ...
    // Enhanced UX preferences (replacing experiment system)
    val enableBottomSheetExpandFully = boolean("enable_bottom_sheet_expand_fully", true)
    val enableBottomSheetUrlDoubleTap = boolean("enable_bottom_sheet_url_double_tap", true)
    val enableInterceptAccidentalTaps = boolean("enable_intercept_accidental_taps", true)
    val enableManualFollowRedirects = boolean("enable_manual_follow_redirects", false)
    val disableBottomSheetStateSave = boolean("disable_bottom_sheet_state_save", false)
    val enableExpressiveLoadingSheet = boolean("enable_expressive_loading_sheet", true)
